...
//        Each line is its own review. Output the sentiment of each review in a bulleted list and then provide a generated sentiment of all reviews.
...
//ChatResponse responseResponse = await client.GetResponseAsync(analysisPrompt);
//Console.WriteLine($"assistant >>>\n{responseResponse}");
#endregion
...
while (true)
{
    ...
    await foreach (ChatResponseUpdate item in client.GetStreamingResponseAsync(chatHistory))
    {
